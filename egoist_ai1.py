...
БЕСПЛАТНЫЙ ПРЕВЬЮ:
1. «КТО Я ✦»
2. «Как ты проявляешься» - сначала коротко объясни, что показывает Асцендент, затем дай синтез.
3. «Твоё ядро» - Солнце главный герой. Учитывай знак, точный дом, связи, диспозитора и связанные сферы, но не перечисляй механику.
4. «Твоя тень» - противоположный знак Солнца как полезное дополнение, а не плохая часть личности.
5. «Что тебе нужно эмоционально» - Луна, её знак, дом и важные связи.
6. «Если коротко» - 3-4 предложения нового синтеза, не повторяющего предыдущие формулировки.
7. В конце блок «🔒 КАК РАСКРЫТЬ СЕБЯ СИЛЬНЕЕ» с четырьмя пунктами: как проявлять себя через Солнце и что зажигает; что гасит; как использовать ресурс тени; как возвращаться к эмоциональному комфорту. Не раскрывай ответы бесплатно.
...
Объём бесплатного текста: примерно 650-950 слов.
Объём полного текста: примерно 800-1200 слов.
...
async def paid(m:Message,state:FSMContext):
    ...
    if sec == "who":
        text = await who_ai_text(p["chart"], paid=True)
