...
# --- EGOIST compatibility and lunar nodes ---
def fmtdeg(deg):
    return format_degree(deg)

def _calculate_nodes(jd_ut):
    xx,_ = swe.calc_ut(jd_ut, swe.TRUE_NODE, swe.FLG_SWIEPH | swe.FLG_SPEED)
    north = point_from_longitude("Северный узел", xx[0])
    south = point_from_longitude("Южный узел", xx[0] + 180.0)
    north.speed = xx[3]
    south.speed = xx[3]
    return north, south
...
def strongest(c, names, limit=4):
    rows=[]; seen=set()
    for name in names:
        if name not in c.get("planets",{}): continue
        for a in c.get("aspects",{}).get(name,[]):
            other=a["planet"]; pair=tuple(sorted((name,other)))
            if pair in seen: continue
            seen.add(pair)
            rows.append({"a":name,"b":other,"aspect":a["aspect"],"orb":a["orb"],"strength":a["strength"],"rank":strength_rank(a["strength"])})
    rows.sort(key=lambda x:(x["rank"],x["orb"]))
    return rows[:limit]
...
def chart(date_str, time_str, lat, lon):
    ...
    c["planets"]["Северный узел"]=north; c["planets"]["Южный узел"]=south
    ...
    # aspects to nodes are calculated against the expanded point set
    c["aspects"]={name: aspects_for(name,c["planets"]) for name in c["planets"]}
    c["sun_dispositor"]=c["sun_ruler_name"]
    c["asc_ruler"]=c["asc_ruler_name"]
    c["date"]=date_str; c["time"]=time_str; c["lat"]=lat; c["lon"]=lon
    return c

def solar_return(natal, year, lat, lon):
    # Search the exact solar longitude return around the birthday, then build Placidus houses for the requested location.
    birth_sun=natal["planets"]["Солнце"].longitude
    birth=datetime.strptime(natal["date"],"%d.%m.%Y")
    guess=datetime(year,birth.month,birth.day,12,0,tzinfo=ZoneInfo("UTC"))
    lo=swe.julday(guess.year,guess.month,guess.day,0.0)-2
    hi=lo+5
    def delta(jd):
        xx,_=swe.calc_ut(jd,swe.SUN,swe.FLG_SWIEPH)
        return ((xx[0]-birth_sun+180)%360)-180
    # find a sign-changing bracket at 3-hour steps
    step=0.125; a=lo; fa=delta(a); bracket=None
    while a+step<=hi:
        b=a+step; fb=delta(b)
        if fa==0 or fa*fb<=0: bracket=(a,b); break
        a,fa=b,fb
    if bracket is None: bracket=(lo,hi)
    a,b=bracket
    for _ in range(50):
        m=(a+b)/2; fm=delta(m); fa=delta(a)
        if fa*fm<=0: b=m
        else: a=m
    jd=(a+b)/2
    y,mo,d,ut=swe.revjul(jd)
    hour=int(ut); minute=int((ut-hour)*60); second=int(round((((ut-hour)*60)-minute)*60))
    if second==60: second=59
    utc=datetime(y,mo,d,hour,minute,second,tzinfo=ZoneInfo("UTC"))
    planets=calculate_planets(jd); cusps,asc,mc=calculate_placidus_houses(jd,lat,lon)
    houses={name:house_of_longitude(point.longitude,cusps) for name,point in planets.items()}
    return {"utc":utc,"planets":planets,"cusps":cusps,"asc":asc,"mc":mc,"houses":houses,"lat":lat,"lon":lon}
