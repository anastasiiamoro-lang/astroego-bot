...
def solar_return(natal, year, lat, lon):
    ...
    try:
        ...
        center = natal_utc.replace(year=int(year), day=28)
...
# =====================================================================
# ПРОГНОСТИЧЕСКИЙ ДВИЖОК И СИНАСТРИЯ
# =====================================================================
...
TRANSIT_PRIORITY = {
    "Плутон": 0, "Нептун": 1, "Уран": 2, "Сатурн": 3,
    "Юпитер": 4, "Северный узел": 5, "Марс": 6,
    "Венера": 7, "Меркурий": 8, "Солнце": 9,
}


def _require_exact_birth_time(c):
    value = str(c.get("time_str") or "").strip()
    if not re.fullmatch(r"(?:[01]\d|2[0-3]):[0-5]\d", value):
        raise ValueError(
            "Для прогностики обязательно точное время рождения в формате ЧЧ:ММ. "
            "Нажми /start и введи данные заново."
        )


def _transit_positions(dt_utc):
    positions = calculate_planets(julian_day(dt_utc))
    north, _ = _calculate_nodes(julian_day(dt_utc))
    positions["Северный узел"] = north
    return positions


def _add_target(targets, name, point):
    if point is not None and name not in targets:
        targets[name] = point


def _forecast_targets(natal, sec):
    planets = natal["planets"]
    cusps = natal["cusps"]
    targets = {}

    if sec == "forecast_love":
        names = ["Венера", "Солнце", "Луна", "Марс", "Сатурн", "Плутон"]
        ruler7 = main_ruler_for_sign(cusp_sign(cusps[6]))
        names.append(ruler7)
        names.extend(name for name, house in natal["houses"].items() if house == 7)
        for name in names:
            _add_target(targets, name, planets.get(name))
        for house in (5, 7, 8):
            _add_target(targets, f"куспид {house} дома", point_from_longitude(f"куспид {house} дома", cusps[house-1]))
        _add_target(targets, "Десцендент", point_from_longitude("Десцендент", cusps[6]))
        return targets

    if sec == "forecast_money":
        names = ["Солнце", "Меркурий", "Венера", "Марс", "Юпитер", "Сатурн", "Плутон"]
        for house in (2, 6, 10):
            names.append(main_ruler_for_sign(cusp_sign(cusps[house-1])))
            _add_target(targets, f"куспид {house} дома", point_from_longitude(f"куспид {house} дома", cusps[house-1]))
        for name in names:
            _add_target(targets, name, planets.get(name))
        _add_target(targets, "MC", natal.get("mc"))
        return targets

    for name, point in planets.items():
        _add_target(targets, name, point)
    _add_target(targets, "Асцендент", natal.get("asc"))
    _add_target(targets, "MC", natal.get("mc"))
    _add_target(targets, "Десцендент", point_from_longitude("Десцендент", cusps[6]))
    return targets


def _transit_orb_limit(transit_name):
    if transit_name in {"Плутон", "Нептун", "Уран", "Сатурн", "Юпитер", "Северный узел"}:
        return 1.6
    if transit_name in {"Марс", "Венера"}:
        return 1.0
    return 0.75


def _scan_transits(natal, sec, start_dt, days):
    """Daily ephemeris scan, grouped into aspect windows around exact hits."""
    targets = _forecast_targets(natal, sec)
    active = {}
    completed = []
    house_rows = []
    last_houses = {}
    ...
    for offset in range(int(days) + 1):
        dt = (start_dt + timedelta(days=offset)).replace(hour=12, minute=0, second=0, microsecond=0)
        positions = _transit_positions(dt)
        seen_today = set()

        for transit_name, transit_point in positions.items():
            if transit_name == "Луна":
                continue
            house = house_of_longitude(transit_point.longitude, natal["cusps"])
            if transit_name not in last_houses:
                last_houses[transit_name] = house
                house_rows.append((dt, transit_name, house, "начало периода"))
            elif last_houses[transit_name] != house:
                last_houses[transit_name] = house
                house_rows.append((dt, transit_name, house, "входит в дом"))

            limit = _transit_orb_limit(transit_name)
            for target_name, target_point in targets.items():
                distance = angular_distance(transit_point.longitude, target_point.longitude)
                for aspect_name, angle in PREDICTIVE_ASPECTS.items():
                    orb = abs(distance - angle)
                    if orb > limit:
                        continue
                    key = (transit_name, target_name, aspect_name)
                    seen_today.add(key)
                    row = active.get(key)
                    if row is None:
                        active[key] = {
                            "transit": transit_name, "target": target_name,
                            "aspect": aspect_name, "start": dt, "end": dt,
                            "exact": dt, "orb": orb,
                            "retrograde": getattr(transit_point, "speed", 0) < 0,
                        }
                    else:
                        row["end"] = dt
                        if orb < row["orb"]:
                            row["orb"] = orb
                            row["exact"] = dt
                            row["retrograde"] = getattr(transit_point, "speed", 0) < 0
    ...
    completed.sort(key=lambda row: (
        TRANSIT_PRIORITY.get(row["transit"], 99),
        row["orb"], row["exact"],
    ))
    return completed[:90], house_rows


def _format_transit_facts(events, house_rows, period_start, period_end):
    lines = [
        f"Период расчёта: {period_start:%d.%m.%Y} - {period_end:%d.%m.%Y}.",
        "Точные транзитные окна (даты рассчитаны по эфемеридам):",
    ]
    for row in events:
        window = f"{row['start']:%d.%m.%Y}-{row['end']:%d.%m.%Y}"
        retro = ", ретроградное движение" if row.get("retrograde") else ""
        lines.append(
            f"- {window}; точнее всего {row['exact']:%d.%m.%Y}: "
            f"транзитный {row['transit']} {row['aspect']} натальный показатель «{row['target']}», "
            f"минимальный орб {row['orb']:.2f}°{retro}."
        )
    lines.append("Прохождения по натальным домам:")
    for dt, planet, house, kind in house_rows:
        lines.append(f"- {dt:%d.%m.%Y}: {planet}, {kind}, {house} дом.")
    return "\n".join(lines)


def _synastry_aspects(chart_a, chart_b):
    rows = []
    major = {"Солнце", "Луна", "Меркурий", "Венера", "Марс", "Юпитер", "Сатурн", "Уран", "Нептун", "Плутон", "Северный узел", "Южный узел"}
    for a_name, a in chart_a["planets"].items():
        if a_name not in major:
            continue
        for b_name, b in chart_b["planets"].items():
            if b_name not in major:
                continue
            distance = angular_distance(a.longitude, b.longitude)
            for aspect_name, angle in PREDICTIVE_ASPECTS.items():
                orb = abs(distance - angle)
                max_orb = 7.0 if (a_name in {"Солнце", "Луна"} or b_name in {"Солнце", "Луна"}) else 5.0
                if orb <= max_orb:
                    rows.append({"a": a_name, "b": b_name, "aspect": aspect_name, "orb": orb})
    rows.sort(key=lambda row: row["orb"])
    return rows[:80]


def _chart_compact_facts(c, owner):
    lines = [f"{owner}: Асцендент {c['asc'].sign}; Десцендент {SIGNS[cusp_sign(c['cusps'][6])]}; MC {c['mc'].sign}."]
    for name, p in c["planets"].items():
        lines.append(f"{owner}, {name}: {p.sign}, {c['houses'].get(name)} дом, долгота {p.longitude:.3f}°.")
    return lines


def _synastry_facts(chart_a, chart_b):
    lines = ["СЛУЖЕБНЫЕ ДАННЫЕ ДВУХ НАТАЛЬНЫХ КАРТ."]
    lines.extend(_chart_compact_facts(chart_a, "Владелец запроса"))
    lines.extend(_chart_compact_facts(chart_b, "Партнёр"))
    lines.append("Межкартные аспекты:")
    for row in _synastry_aspects(chart_a, chart_b):
        lines.append(f"- {row['a']} владельца: {row['aspect']} к {row['b']} партнёра, орб {row['orb']:.2f}°.")
    lines.append("Попадания планет партнёра в дома владельца:")
    for name, p in chart_b["planets"].items():
        lines.append(f"- {name} партнёра попадает в {house_of_longitude(p.longitude, chart_a['cusps'])} дом владельца.")
    lines.append("Попадания планет владельца в дома партнёра:")
    for name, p in chart_a["planets"].items():
        lines.append(f"- {name} владельца попадает в {house_of_longitude(p.longitude, chart_b['cusps'])} дом партнёра.")
    return "\n".join(lines)


def _solar_facts(natal, solar, year, city):
    asc_ruler = main_ruler_for_sign(solar["asc"].sign_index)
    mc_ruler = main_ruler_for_sign(solar["mc"].sign_index)
    lines = [
        f"Соляр на {year} год. Город встречи дня рождения: {city}.",
        f"Точный момент возвращения Солнца: {solar['utc']:%d.%m.%Y %H:%M} UTC.",
        f"Асцендент соляра: {solar['asc'].sign} {format_degree(solar['asc'].degree)}, попадает в {house_of_longitude(solar['asc'].longitude, natal['cusps'])} натальный дом.",
        f"MC соляра: {solar['mc'].sign} {format_degree(solar['mc'].degree)}, попадает в {house_of_longitude(solar['mc'].longitude, natal['cusps'])} натальный дом.",
        f"Управитель Асцендента соляра: {asc_ruler}, {solar['planets'][asc_ruler].sign}, {solar['houses'].get(asc_ruler)} дом соляра.",
        f"Управитель MC соляра: {mc_ruler}, {solar['planets'][mc_ruler].sign}, {solar['houses'].get(mc_ruler)} дом соляра.",
        "Планеты соляра:"
    ]
    for name, p in solar["planets"].items():
        lines.append(
            f"- {name}: {p.sign} {format_degree(p.degree)}, {solar['houses'].get(name)} дом соляра, "
            f"активирует {house_of_longitude(p.longitude, natal['cusps'])} натальный дом."
        )
    lines.append("Основные аспекты внутри соляра:")
    solar_items = list(solar["planets"].items())
    for i, (a_name, a_point) in enumerate(solar_items):
        for b_name, b_point in solar_items[i+1:]:
            distance = angular_distance(a_point.longitude, b_point.longitude)
            for aspect_name, angle in PREDICTIVE_ASPECTS.items():
                orb = abs(distance - angle)
                if orb <= 4.0:
                    lines.append(f"- {a_name}: {aspect_name} к {b_name}, орб {orb:.2f}°.")
    lines.append("Аспекты планет соляра к натальным показателям:")
    for s_name, s_point in solar["planets"].items():
        for n_name, n_point in natal["planets"].items():
            distance = angular_distance(s_point.longitude, n_point.longitude)
            for aspect_name, angle in PREDICTIVE_ASPECTS.items():
                orb = abs(distance - angle)
                if orb <= 2.0:
                    lines.append(f"- {s_name} соляра: {aspect_name} к натальному {n_name}, орб {orb:.2f}°.")
    return "\n".join(lines)
...
LABEL={
    ...
    "forecast_now":"⚡ Что со мной происходит сейчас?",
    "forecast_3m":"🔮 Что ждёт меня в ближайшие 3 месяца?",
    "forecast_love":"❤️ Что ждёт меня в любви?",
    "forecast_money":"💰 Что ждёт меня в деньгах и карьере?",
    "solar_year":"☀️ Мой личный год - соляр",
    "synastry":"💞 Совместимость с партнёром",
}

INVOICE_LABELS={
    "forecast_now":"Текущий период",
    "forecast_3m":"Прогноз на 3 месяца",
    "forecast_love":"Прогноз в любви",
    "forecast_money":"Деньги и карьера",
    "solar_year":"Личный год - соляр",
...
PRODUCT_DESCRIPTIONS={
    "forecast_now":(
        ...
        "Узнай, какой жизненный период ты проходишь, почему происходят перемены и какая сфера сейчас требует внимания. "
        "Внутри - главные транзиты, возможности, напряжения и сроки, когда ситуация начнёт меняться.\n\n"
        "Для прогностики обязательно точное время рождения."
    ),
    "forecast_3m":(
        "🔮 ЧТО ЖДЁТ МЕНЯ В БЛИЖАЙШИЕ 3 МЕСЯЦА?\n\n"
        "Персональный прогноз главных событий: отношения, работа, деньги, семья, поездки и внутреннее состояние. "
        "Внутри - важные периоды, возможные перемены, лучшие моменты для действий и рекомендации.\n\n"
        "Для прогностики обязательно точное время рождения."
    ),
    "forecast_love":(
        "❤️ ЧТО ЖДЁТ МЕНЯ В ЛЮБВИ?\n\n"
        "Прогноз личной жизни на ближайшие 6 месяцев с учётом Венеры, VII дома и индивидуальных транзитов. "
        "Ты узнаешь о перспективах знакомства, развитии отношений, важных решениях и сильных любовных периодах.\n\n"
        "Для прогностики обязательно точное время рождения."
    ),
    "forecast_money":(
        "💰 ЧТО ЖДЁТ МЕНЯ В ДЕНЬГАХ И КАРЬЕРЕ?\n\n"
        "Финансовый и карьерный прогноз на ближайшие 6 месяцев: возможности роста, смена работы, проекты, переговоры, доходы и риски. "
        "Внутри - благоприятные периоды для действий и даты, когда стоит проявить осторожность.\n\n"
        "Для прогностики обязательно точное время рождения."
    ),
    "solar_year":(
        "☀️ МОЙ ЛИЧНЫЙ ГОД - СОЛЯР\n\n"
        "Подробный прогноз от одного дня рождения до следующего: любовь, деньги, карьера, семья, здоровье, поездки и главные перемены. "
        "Внутри - тема года, прогноз по периодам, сильные возможности, напряжённые моменты и важные даты.\n\n"
        "Обязательны точное время рождения и город, в котором ты встречаешь день рождения."
    ),
    "synastry":(
        "💞 СОВМЕСТИМОСТЬ С ПАРТНЁРОМ\n\n"
        "Глубокий разбор двух натальных карт: чувства, притяжение, общение, сексуальная совместимость, конфликты и перспективы союза. "
        "Ты узнаешь, что вас соединяет, где находятся слабые места пары и как сделать отношения гармоничнее."
    ),
}
...
class SolarInput(StatesGroup): year=State(); city=State()
class PartnerInput(StatesGroup): date=State(); time=State(); city=State()
...
def menu():
    rows=[
      ...
      [InlineKeyboardButton(text="🔮 Прогностика",callback_data="forecast:menu")],
      [InlineKeyboardButton(text="💞 Совместимость с партнёром",callback_data="sec:synastry")],
...
def forecast_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
      [InlineKeyboardButton(text="⚡ Что со мной происходит сейчас? · 600⭐",callback_data="sec:forecast_now")],
      [InlineKeyboardButton(text="🔮 Ближайшие 3 месяца · 600⭐",callback_data="sec:forecast_3m")],
      [InlineKeyboardButton(text="❤️ Что ждёт меня в любви? · 600⭐",callback_data="sec:forecast_love")],
      [InlineKeyboardButton(text="💰 Деньги и карьера · 800⭐",callback_data="sec:forecast_money")],
      [InlineKeyboardButton(text="☀️ Мой личный год - соляр · 1000⭐",callback_data="sec:solar_year")],
      [InlineKeyboardButton(text="← Назад",callback_data="menu")],
    ])

def love_status_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
      [InlineKeyboardButton(text="Я свободна / свободен",callback_data="love_status:single")],
      [InlineKeyboardButton(text="Я в отношениях",callback_data="love_status:relationship")],
      [InlineKeyboardButton(text="Всё сложно",callback_data="love_status:complicated")],
      [InlineKeyboardButton(text="← Назад",callback_data="forecast:menu")],
    ])
...
def unlock_kb(sec):
    back = "forecast:menu" if sec.startswith("forecast_") or sec == "solar_year" else "menu"
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"Открыть полный разбор · ⭐ {PRICES[sec]}",callback_data=f"buy:{sec}")],
...
def _safe_ai_unavailable(sec, paid=False):
    titles = {
        ...
        "child": "ПОТЕНЦИАЛ РЕБЁНКА 🌱",
        "forecast_now": "ЧТО СО МНОЙ ПРОИСХОДИТ СЕЙЧАС? ⚡",
        "forecast_3m": "БЛИЖАЙШИЕ 3 МЕСЯЦА 🔮",
        "forecast_love": "ЧТО ЖДЁТ МЕНЯ В ЛЮБВИ? ❤️",
        "forecast_money": "ДЕНЬГИ И КАРЬЕРА 💰",
        "solar_year": "МОЙ ЛИЧНЫЙ ГОД - СОЛЯР ☀️",
        "synastry": "СОВМЕСТИМОСТЬ С ПАРТНЁРОМ 💞",
    }
    return (
...
PREDICTION_COMMON_INSTRUCTIONS = COMMON_AI_INSTRUCTIONS + """

ДОПОЛНИТЕЛЬНЫЕ ПРАВИЛА ДЛЯ ПРОГНОСТИКИ:
- это прогноз, а не натальный психологический портрет;
- сначала определи натальную основу темы, затем наложи рассчитанные транзиты;
- медленные планеты задают сюжет и длительный этап, быстрые планеты уточняют даты и включают события;
- не объявляй крупное событие по одному короткому транзиту: для сильного вывода ищи 2-3 независимых подтверждения;
- различай гармоничные возможности, напряжённые проверки и смешанные трансформационные периоды;
- не выдавай вероятность за гарантию и не запугивай клиента;
- называй конкретные даты и интервалы только из входных расчётных данных;
- не придумывай затмения, прогрессии, дирекции и аспекты, которых нет во входе;
- покажи, как один период может проявиться по-разному в зависимости от реальной ситуации человека;
- техническую кухню используй внутри, клиенту объясняй простым человеческим языком;
- прогноз должен отвечать: что происходит, чего ожидать, когда вероятнее проявление и как лучше действовать.

Верни только готовый клиентский текст для Telegram.
"""

PREDICTION_SECTION_RULES = {
    "forecast_now": """
ЗАГОЛОВОК: «ЧТО СО МНОЙ ПРОИСХОДИТ СЕЙЧАС? ⚡».
Дай содержательный прогноз текущего этапа: 3-5 главных тем, активированные сферы, возможности, напряжение, ближайший поворот и срок ослабления/смены текущего влияния. В конце добавь практический раздел «Как лучше прожить этот период». Не перечисляй все транзиты подряд.
""",
    "forecast_3m": """
ЗАГОЛОВОК: «БЛИЖАЙШИЕ 3 МЕСЯЦА 🔮».
Сначала дай главную линию трёх месяцев. Затем последовательно разберись по периодам и сферам: отношения, работа и деньги, семья/дом, энергия, поездки/обучение. Выдели лучшие окна для действий и осторожные периоды с точными датами из расчёта. Заверши коротким итогом и стратегией на три месяца.
""",
    "forecast_love": """
ЗАГОЛОВОК: «ЧТО ЖДЁТ МЕНЯ В ЛЮБВИ? ❤️».
Горизонт - 6 месяцев. Обязательно начни с натальной основы: Венера, её знак/дом/аспекты и диспозитор; VII дом, планеты в нём, управитель VII дома, его знак/дом/аспекты; связи Солнца с Венерой и управителем VII дома. Дополнительно учитывай Луну, Марс, V и VIII дома, когда они подтверждают вывод.

В прогностике обязательно синтезируй транзиты к Венере, управителю VII дома, Десценденту, планетам VII дома и прохождения по V, VII и VIII домам. Особое внимание - Юпитеру, Сатурну, Урану, Нептуну и Плутону; Венеру и Марс используй как активаторы дат.

Не обещай отношения только из-за прохода Венеры по VII дому. Сильное любовное окно выделяй при 2-3 подтверждениях. Плутон не называй автоматически расставанием: он может дать сильное притяжение, ревность, власть, зависимость, кризис, завершение либо глубокую перестройку союза - выбери варианты по общей картине.

Учти указанный статус личной жизни. Структура: что происходит сейчас; главная перспектива; вероятные знакомства/развитие союза; лучшие любовные окна; напряжённые периоды; важные решения; рекомендации. Все даты бери только из расчёта.
""",
    "forecast_money": """
ЗАГОЛОВОК: «ДЕНЬГИ И КАРЬЕРА: БЛИЖАЙШИЕ 6 МЕСЯЦЕВ 💰».
Сначала определи натальную финансово-карьерную основу по II, VI и X домам, их управителям, планетам внутри, Солнцу, Венере, Юпитеру, Сатурну и MC. Затем синтезируй транзиты к этим показателям и прохождения по II, VI и X домам.

Разбери: текущий этап; доход и расходы; работа/карьера; проекты и переговоры; возможности роста; риски и задержки; лучшие окна для действий; периоды осторожности. Не обещай конкретную сумму или увольнение без нескольких подтверждений. Дай даты и практические рекомендации.
""",
    "solar_year": """
ЗАГОЛОВОК: «МОЙ ЛИЧНЫЙ ГОД - СОЛЯР ☀️».
Это прогноз от одного дня рождения до следующего. Обязательно анализируй Асцендент и MC соляра, управителя Асцендента, планеты по точным домам соляра, основные аспекты и то, какие натальные дома они активируют. Не путай дома соляра с натальными домами.

Структура: главная тема года; личность и внутренние изменения; любовь; карьера и деньги; семья/дом; здоровье и энергия без медицинских диагнозов; поездки и обучение; друзья/отдых/творчество; возможности; напряжённые зоны; развитие года по периодам; ключевые рекомендации. Показывай плюсы, минусы и варианты проявления, а не фатальные события.
""",
    "synastry": """
ЗАГОЛОВОК: «СОВМЕСТИМОСТЬ С ПАРТНЁРОМ 💞».
Это синастрия двух натальных карт, не прогноз. Сначала коротко опиши любовную модель каждого, затем синтезируй межкартные аспекты и взаимные попадания планет в дома.

Обязательно разберись: что притягивает; как каждый воспринимает другого; эмоциональная совместимость; проявление любви; сексуальное притяжение; общение; конфликты и триггеры; ревность/зависимость/борьба за власть при подтверждении; быт и совместная жизнь; Сатурн и устойчивость; потенциал длительного союза; сильные стороны; конкретные рекомендации каждому.

Не выводи процент совместимости и не делай вывод о всей паре по одному аспекту. Не обещай брак и не объявляй расставание. Покажи, где союз естественно поддерживает людей, а где требует зрелости и договорённостей.
""",
}


def _prediction_facts(c, sec, profile):
    _require_exact_birth_time(c)
    now = datetime.now(timezone.utc)

    if sec == "synastry":
        partner = profile.get("partner_chart")
        if not partner:
            raise ValueError("Сначала нужны данные рождения партнёра.")
        return _synastry_facts(c, partner)

    if sec == "solar_year":
        year = int(profile["solar_year"])
        lat = float(profile["solar_lat"])
        lon = float(profile["solar_lon"])
        city = profile["solar_city"]
        solar = solar_return(c, year, lat, lon)
        events, house_rows = _scan_transits(c, "forecast_3m", solar["utc"], 365)
        return (
            _section_ai_facts(c, "forecast_3m", paid=True)
            + "\n\n" + _solar_facts(c, solar, year, city)
            + "\n\n" + _format_transit_facts(events, house_rows, solar["utc"], solar["utc"] + timedelta(days=365))
        )

    if sec == "forecast_now":
        start = now - timedelta(days=14)
        days = 60
        natal_sec = "forecast_3m"
    elif sec == "forecast_3m":
        start = now
        days = 92
        natal_sec = "forecast_3m"
    elif sec == "forecast_love":
        start = now
        days = 183
        natal_sec = "love"
    elif sec == "forecast_money":
        start = now
        days = 183
        natal_sec = "career_money"
    else:
        raise KeyError(sec)

    events, house_rows = _scan_transits(c, sec, start, days)
    facts = [
        _section_ai_facts(c, natal_sec, paid=True),
        _format_transit_facts(events, house_rows, start, start + timedelta(days=days)),
    ]
    if sec == "forecast_love":
        statuses = {
            "single": "человек сейчас свободен",
            "relationship": "человек сейчас в отношениях",
            "complicated": "в личной жизни сейчас сложная/неопределённая ситуация",
        }
        facts.append("Текущий статус: " + statuses.get(profile.get("love_status"), "не указан"))
    return "\n\n".join(facts)


async def prediction_ai_text(c, sec, profile):
    if sec not in PREDICTION_SECTION_RULES:
        raise KeyError(sec)
    key = os.getenv("OPENAI_API_KEY", "").strip()
    if not key:
        return _safe_ai_unavailable(sec, True)
    try:
        facts = await asyncio.to_thread(_prediction_facts, c, sec, profile)
        client = AsyncOpenAI(api_key=key, timeout=120.0)
        response = await client.responses.create(
            model=os.getenv("OPENAI_MODEL", "gpt-5.6-terra"),
            instructions=PREDICTION_COMMON_INSTRUCTIONS + "\n\n" + PREDICTION_SECTION_RULES[sec],
            input=facts,
            max_output_tokens=6500,
        )
        result = _ai_text_qa(response.output_text)
        if len(result) < 700:
            raise ValueError("AI prediction is unexpectedly short")
        return result
    except Exception as e:
        print(f"[EGOIST AI] {sec.upper()} generation failed: {type(e).__name__}: {e}")
        return _safe_ai_unavailable(sec, True)
...
async def bdate(m:Message,state:FSMContext):
    await state.update_data(date=m.text.strip()); await m.answer("Точное время рождения обязательно для прогностики. Введи его, например 14:35:"); await state.set_state(Birth.time)
...
@router.callback_query(F.data=="forecast:menu")
async def open_forecast_menu(q:CallbackQuery):
    await q.message.answer(
        "🔮 ПРОГНОСТИКА\n\n"
        "Выбери интересующий прогноз. Для всех прогностических разделов обязательно точное время рождения: "
        "без него невозможно корректно определить дома, управителей и индивидуальные периоды событий.",
...
async def section(q:CallbackQuery,state:FSMContext):
    ...
    if sec in predictive:
        if not has_access(q.from_user.id, sec):
            ...
            await q.answer()
            return
        if sec == "forecast_love" and not p.get("love_status"):
            await q.message.answer("Чтобы прогноз был точнее, выбери, что сейчас происходит в личной жизни:", reply_markup=love_status_kb())
            await q.answer()
            return
        if sec == "solar_year" and not p.get("solar_year"):
            await q.message.answer("На какой год построить соляр? Введи год четырьмя цифрами, например 2027:")
            await state.set_state(SolarInput.year)
            await q.answer()
            return
        if sec == "synastry" and not p.get("partner_chart"):
            await q.message.answer("Введи дату рождения партнёра в формате ДД.ММ.ГГГГ:")
            await state.set_state(PartnerInput.date)
            await q.answer()
            return
        try:
            text = await prediction_ai_text(p["chart"], sec, p)
            await send_long(q.message.answer, text, reply_markup=menu())
        except Exception as e:
            await q.message.answer(f"Не получилось открыть прогноз: {e}")
        await q.answer()
        return
...
async def paid(m:Message,state:FSMContext):
    ...
    if sec == "forecast_love":
        await m.answer("Чтобы прогноз был точнее, выбери, что сейчас происходит в личной жизни:", reply_markup=love_status_kb())
        return

    if sec == "solar_year":
        await m.answer("На какой год построить соляр? Введи год четырьмя цифрами, например 2027:")
        await state.set_state(SolarInput.year)
        return

    if sec == "synastry":
        await m.answer("Введи дату рождения партнёра в формате ДД.ММ.ГГГГ:")
        await state.set_state(PartnerInput.date)
        return

    if sec in {"forecast_now", "forecast_3m", "forecast_money"}:
        text = await prediction_ai_text(p["chart"], sec, p)
...
async def love_status_selected(q:CallbackQuery):
    ...
    if status not in {"single", "relationship", "complicated"}:
        await q.answer("Не удалось определить вариант.", show_alert=True)
        return
    p = PROFILES.get(q.from_user.id)
    if not p:
        await q.message.answer("Сначала нажми /start и введи данные рождения.")
        await q.answer()
        return
    p["love_status"] = status
    await q.answer()
    await q.message.answer("Собираю твой прогноз личной жизни. Это может занять несколько секунд ✨")
    text = await prediction_ai_text(p["chart"], "forecast_love", p)
    await send_long(q.message.answer, text, reply_markup=menu())


@router.message(SolarInput.year)
async def solar_year_input(m:Message,state:FSMContext):
    raw = m.text.strip()
    if not re.fullmatch(r"\d{4}", raw):
        await m.answer("Введи год четырьмя цифрами, например 2027:")
        return
    year = int(raw)
    current_year = datetime.now(timezone.utc).year
    if year < current_year - 1 or year > current_year + 5:
        await m.answer(f"Можно выбрать год от {current_year - 1} до {current_year + 5}. Введи нужный год:")
        return
    await state.update_data(solar_year=year)
    await m.answer(
        "В каком городе ты будешь находиться в день рождения? "
        "Для соляра важен именно город встречи дня рождения, например: Москва, Россия"
    )
    await state.set_state(SolarInput.city)


@router.message(SolarInput.city)
async def solar_city_input(m:Message,state:FSMContext):
    p = PROFILES.get(m.from_user.id)
    if not p:
        await state.clear()
        await m.answer("Сначала нажми /start и введи данные рождения.")
        return
    data = await state.get_data()
    city = m.text.strip()
    try:
        lat, lon = await locate(city)
        p.update({
            "solar_year": int(data["solar_year"]),
            "solar_city": city,
            "solar_lat": lat,
            "solar_lon": lon,
        })
        await state.clear()
        await m.answer("Строю соляр и собираю прогноз на личный год ✨")
        text = await prediction_ai_text(p["chart"], "solar_year", p)
        await send_long(m.answer, text, reply_markup=menu())
    except Exception as e:
        await m.answer(f"Не получилось определить город или построить соляр: {e}")


@router.message(PartnerInput.date)
async def partner_date_input(m:Message,state:FSMContext):
    await state.update_data(partner_date=m.text.strip())
    await m.answer(
        "Введи точное время рождения партнёра, например 14:35. "
        "Оно нужно для домов и взаимных попаданий планет:"
    )
    await state.set_state(PartnerInput.time)


@router.message(PartnerInput.time)
async def partner_time_input(m:Message,state:FSMContext):
    value = m.text.strip()
    if not re.fullmatch(r"(?:[01]\d|2[0-3]):[0-5]\d", value):
        await m.answer("Нужно точное время в формате ЧЧ:ММ, например 09:40:")
        return
    await state.update_data(partner_time=value)
    await m.answer("Введи город рождения партнёра и страну:")
    await state.set_state(PartnerInput.city)


@router.message(PartnerInput.city)
async def partner_city_input(m:Message,state:FSMContext):
    p = PROFILES.get(m.from_user.id)
    if not p:
        await state.clear()
        await m.answer("Сначала нажми /start и введи свои данные рождения.")
        return
    data = await state.get_data()
    city = m.text.strip()
    try:
        lat, lon = await locate(city)
        partner = chart(data["partner_date"], data["partner_time"], lat, lon)
        p["partner_chart"] = partner
        p["partner_birth_city"] = city
        await state.clear()
        await m.answer("Сопоставляю две натальные карты и собираю синастрию ✨")
        text = await prediction_ai_text(p["chart"], "synastry", p)
        await send_long(m.answer, text, reply_markup=menu())
    except Exception as e:
        await m.answer(f"Не получилось построить карту партнёра: {e}")
