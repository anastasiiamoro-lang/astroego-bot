...
Это символическая астрологическая интерпретация, а не доказанный факт о прошлых жизнях. В начале блока в 1-2 коротких фразах объясни простыми словами, как в астрологии читают кармические узлы и почему они важны: Южный узел показывает знакомые, привычные способы действовать и уже наработанные качества, Северный - направление развития, которое постепенно осваивается. Затем переходи к персональному разбору. Анализируй Северный и Южный узлы по знакам и точным домам, их ось, соединения и существенные аспекты, управителей знаков узлов, а также связи с Солнцем, Луной, Асцендентом, MC, стеллиумами и доминирующими темами. Не пугай и не используй фатализм.

БЕСПЛАТНЫЙ ПРЕВЬЮ:
1. «С чем ты пришла» - привычные способности и сценарии Южного узла.
2. «Куда тебя ведёт жизнь» - направление Северного узла.
3. «Что притягивается в твою жизнь» - повторяющиеся темы и ситуации, если карта это подтверждает. Покажи, где знакомый паттерн удобен, но может удерживать на месте, чтобы естественно возник вопрос «как выйти из повтора?».
...
ПОЛНЫЙ ПЛАТНЫЙ РАЗБОР:
Практически раскрой переход от Южного к Северному узлу как ответ на повторяющиеся паттерны, которые бесплатный разбор подсветил бы по этой карте. Объясни, почему привычный сценарий так легко включается, где он полезен, где удерживает на месте и КАК менять выбор. Дай конкретные действия, навыки, способы использовать прошлый опыт без застревания и признаки движения в нужную сторону. Не повторяй бесплатную часть дословно.
""",
    "growth": """
