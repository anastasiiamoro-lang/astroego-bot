...
PLANET_INSTR = {
    ...
    "Нептун":"Нептуном","Плутон":"Плутоном","Северный узел":"Северным узлом",
...
def _planet_aspect_sentence(c, planet, domain="общую тему"):
    parts=[]
    for row in _first_meaningful_aspects(c,planet,2,5.5):
        other=row.get("planet")
        asp=row.get("aspect")
        mod=_aspect_modifier(planet,row)
        if mod:
            parts.append(f"{asp} {planet_phrase_with(other)} {mod}")
    if not parts:
        return ""
    return f" На {domain} особенно влияют связи: " + "; ".join(parts) + "."
...
def who_preview(c):
    ...
    asc_extra += _planet_aspect_sentence(c,asc_ruler,"то, как ты проявляешься")
