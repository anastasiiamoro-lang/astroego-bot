...
WHO_SIGN_MEANINGS = {'Овен': {'sun_core': ['инициатива, прямота, смелость, самостоятельность',
 ...
 'Водолей': {'sun_core': ['независимость мышления, оригинальность, свобода, ориентация на идеи и будущее',
             ...
             'shadow': 'Лев',
             'shadow_resource': 'Ресурс тени - право быть заметным лично, проявлять сердце, творчество, гордость за '
                                'себя и не растворяться полностью в группе или идее.',
...
def _planet_aspect_sentence(c, planet, domain="общую тему"):
    ...
    for row in _first_meaningful_aspects(c,planet,2,5.5):
        ...
        if mod:
            parts.append(f"{asp} с {other} {mod}")
    if not parts:
        return ""
...
def who_preview(c):
    ...
    if ruled:
        disp_text += " Дополнительно через него связаны темы " + ", ".join(f"{h} дома" for h in ruled) + "."
    ...
    return (
        "КТО Я ✦\n\n"
        "Как ты проявляешься\n\n"
        "Асцендент показывает, как ты проявляешься внешне, как входишь в новые ситуации и какое первое впечатление производишь на людей. "
        ...
        "Если коротко\n\n"
        f"Снаружи ты чаще проявляешь качества {SIGN_GEN[asc.sign]}, ядро личности строится по {SIGN_GEN[sun.sign]}, "
...
def nodes_deep(c):
    ...
    return (
        ...
        f"В повторяющейся ситуации отделяй вопрос «что мне уже легко по {SIGN_GEN[sn.sign]}?» от вопроса «какого нового качества {SIGN_GEN[nn.sign]} требует эта ситуация?». "
