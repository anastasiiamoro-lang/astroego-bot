...
WHO_SIGN_MEANINGS = {'Овен': {'sun_core': ['инициатива, прямота, смелость, самостоятельность',
 ...
 'Телец': {'sun_core': ['устойчивость, практичность, терпение, способность создавать материальную опору',
           ...
           'shadow_extreme': 'Перекос возможен между чрезмерным удержанием привычного и резкими внутренними кризисами, '
 ...
 'Рак': {'sun_core': ['чувствительность, память, забота, сильная связь с близкими и личным пространством',
         ...
         'shadow': 'Козерог',
         'shadow_resource': 'Ресурс тени - внутренняя взрослость, границы, самостоятельность, умение действовать даже '
 ...
 'Лев': {'sun_core': ['яркость, творческая воля, достоинство, потребность проявлять индивидуальность',
         ...
         'asc': 'Может производить впечатление заметного, уверенного, тёплого или внутренне собранного человека.',
 ...
 'Скорпион': {'sun_core': ['глубина, воля, наблюдательность, способность видеть скрытые мотивы',
                           'потребность в настоящей интенсивности, честности и внутренней силе',
              ...
              'moon_comfort': 'Эмоционально важны доверие, глубина, честность и ощущение, что близость настоящая, а не '
                              'формальная.',
              'moon_stress': 'В стрессе может усиливаться подозрительность, контроль, внутреннее напряжение или страх '
                             'потери влияния.',
              'shadow': 'Телец',
 ...
 'Козерог': {'sun_core': ['ответственность, амбиции, выдержка, ориентация на результат и долгий путь',
                          ...
                          'сила проявляется через дисциплину, внутренний стержень и способность выдерживать большие '
                          'задачи'],
 ...
 'Рыбы': {'sun_core': ['чувствительность, интуиция, воображение, эмпатия, способность чувствовать невидимые связи',
                       'потребность в смысле, вдохновении и пространстве для внутреннего мира',
...
WHO_PAID_POINTS = ['как проявлять себя через своё Солнце и что тебя по-настоящему зажигает;',
 ...
 'как справляться со стрессом, возвращаться к внутреннему комфорту и находить свой ресурс.']
...
LOVE_VENUS_DIGNITY_MEANING = {'обитель': 'Венерианские темы обычно проживаются естественнее: человеку легче чувствовать вкус, ценность, '
 ...
 'изгнание': 'Любовь и удовольствие могут проживаться более напряжённо, резко или интенсивно. Человеку бывает сложнее '
             'просто принимать приятное без борьбы, крайностей или внутреннего напряжения.',
...
CAREER_SUN_HOUSES = {1: 'талант проявляется через личную инициативу, образ, самостоятельность и прямое влияние',
 ...
 4: 'талант может раскрываться через дом, семью, недвижимость, внутреннюю опору и работу из личного пространства',
...
NODE_PLANETS_FULL = {'Солнце': {'south': 'Знакомый опыт личной силы, лидерства и самовыражения.',
 ...
 'Плутон': {'south': 'Знакомый опыт кризисов, власти, контроля, трансформации, риска и чужих ресурсов.',
            'north': 'Развитие через внутреннюю силу, трансформацию, кризисоустойчивость, влияние и глубокие '
                     'изменения.'}}
...
GROWTH_PLANET_LOGIC = {'Солнце': {'question': 'Что мешает тебе проявляться, чувствовать уверенность и занимать своё место?',
            'resource': 'сильное Солнце даёт внутреннюю опору, волю, жизненность, творчество и ясное ощущение себя',
...
GROWTH_BALANCE_EXAMPLES = {'venus_scorpio': {'tension': 'Венера в Скорпионе может проживать любовь через крайность, сильную привязанность, '
 ...
 'moon_capricorn': {'tension': 'Луна в Козероге может слишком быстро брать себя в руки, запрещать себе слабость и '
                    ...
                    'keep': 'Сохранять собранность, ответственность, внутренний стержень и способность выдерживать '
                            'сложные периоды.',
...
GROWTH_ASPECT_PATTERNS = {'sun_saturn': {'tension': 'страх ошибки, жёсткая внутренняя оценка, ощущение, что признание нужно заслужить',
...
PLANET_ASPECT_SYMBOLISM = {
    "Солнце": {
        ...
        "tense": "может создавать внутренний конфликт вокруг самооценки, воли и права проявляться так, как хочется",
    },
    "Луна": {
        "supportive": "помогает лучше понимать свои чувства, восстанавливаться и чувствовать внутреннюю устойчивость",
    ...
    "Марс": {
        "supportive": "добавляет смелость, инициативу, скорость действий и способность защищать свои интересы",
        "tense": "может давать импульсивность, раздражительность, конфликты, борьбу за контроль и перегрузку от постоянного внутреннего напряжения",
    },
    "Юпитер": {
        "supportive": "даёт широкий взгляд, веру в возможности, амбициозность, тягу к росту, большим целям, обучению и расширению горизонтов",
    ...
    "Сатурн": {
        "supportive": "даёт выдержку, дисциплину, ответственность, серьёзность и способность строить результат надолго",
        "tense": "может создавать страх ошибки, внутреннюю жёсткость, ощущение ограничений, задержек и необходимости многое доказывать через усилие",
    },
    "Уран": {
        "supportive": "даёт оригинальность, независимость, быстрые инсайты, интерес к новому и способность находить нестандартные решения",
        "tense": "может давать резкость, непредсказуемость, внутренний протест против ограничений и сложности выдерживать рутину",
    },
    "Нептун": {
    ...
    "Плутон": {
        "supportive": "даёт психологическую глубину, сильную волю, способность проходить кризисы и влиять на сложные процессы",
...
def _planet_aspect_sentence(c, planet, domain="общую тему"):
    ...
    if not rows:
        return ""
    parts = [aspect_effect_text(row) for row in rows]
    if len(parts) == 1:
        return f" На {domain} особенно влияет {parts[0]}."
    return f" На {domain} особенно влияют две важные связи: " + "; ".join(parts) + "."


def _house_ruler(c, house):
    """Основной управитель знака на куспиде дома."""
    return RULERS[_cusp_sign(c, house)]

def _node_conjunctions(c, node_name, orb=6.0):
    """Планеты в соединении с Северным или Южным узлом."""
    node = _p(c, node_name)
    found = []
...
HOUSE_GENITIVE = {1: 'личности, самостоятельности, образа и способа проявляться', 2: 'денег, имущества, самоценности и личных ресурсов', 3: 'обучения, речи, языков, письма, информации, поездок и связей', 4: 'дома, семьи, родителей, корней, недвижимости и внутренней безопасности', 5: 'творчества, любви, детей, сцены, хобби и удовольствия', 6: 'работы, здоровья, мастерства, ежедневных навыков и режима', 7: 'партнёрства, брака, клиентов, договоров и взаимодействия один на один', 8: 'кризисов, интимности, чужих ресурсов, инвестиций и трансформации', 9: 'высшего образования, путешествий, иностранной среды, философии и преподавания', 10: 'карьеры, статуса, управления, репутации и общественной реализации', 11: 'друзей, сообществ, аудитории, технологий и коллективных проектов', 12: 'уединения, закулисья, творчества, помощи и скрытых процессов'}
...
def who_preview(c):
    ...
    disp_text += _planet_aspect_sentence(c,disp,"реализацию твоего Солнца")

    st_text=""
    if sh in stelliums:
        names=", ".join(stelliums[sh])
        st_text=f" Солнце находится в сильно заполненном {sh} доме вместе с {names}, поэтому эта жизненная сфера становится одной из центральных тем карты."
    ...
    return (
        ...
        f"{WHO_SUN_HOUSES[sh]} "
        f"Тебя особенно зажигают {sun_ignite}.{sun_aspects}{st_text}\n\n"
        f"{disp_text}\n\n"
        "Твоя тень\n\n"
        ...
        f"Дом Луны добавляет сферу {HOUSE_SYMBOL[mh]}, поэтому именно события здесь особенно легко затрагивают внутреннее состояние."
...
def growth_preview(c):
    ...
    return (
        ...
        "Напряжённая планета часто сначала пытается решить ситуацию своим привычным способом, а при перегрузе уходит в крайность: контроль, избегание, импульс, закрытость, идеализацию или внутреннюю жёсткость. "
        ...
        + "\n\nЕсли коротко\n\n"
        "Точки роста - это не список недостатков. Это места, где у тебя больше внутреннего напряжения и одновременно больше энергии для развития."
        + _bullets(GROWTH_PAID_TITLE, GROWTH_PAID_POINTS)
    )
...
def russian_language_qa(text):
    # Final safety net for the most common generated case errors.
    fixes = {
        "с Плутон":"с Плутоном",
        "с Нептун":"с Нептуном",
        "с Уран":"с Ураном",
        ...
        "по Овна":"по Овну",
        "по Рака":"по Раку",
        "проявлять сердце":"проявлять сердечность и щедрость",
    }
    for a,b in fixes.items():
        text=text.replace(a,b)
    # Если где-то всё же появился технический оборот с управлением домом, склоняем корректно.
    text = re.sub(r"управляет (\\d+) дома", r"управляет \\1 домом", text)
    # Убираем случайное двойное окончание после инструментального падежа.
    text = re.sub(r"(Юпитером|Ураном|Нептуном|Плутоном|Марсом|Сатурном|Меркурием|Солнцем|Асцендентом)ом\\b", r"\\1", text)
    return text
...
async def bcity(m:Message,state:FSMContext):
    ...
    try:
        ...
        await send_long(m.answer, new_deep(c, "who") if has_unlock(m.from_user.id, "who") else new_preview(c, "who"), reply_markup=who_markup)
