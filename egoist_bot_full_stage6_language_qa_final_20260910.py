...
def who_preview(c):
    ...
    if ruled:
        ruled_text = ", ".join(f"{h} дома" for h in ruled)
        disp_text += (
            f" При этом {disp} управляет {ruled_text}. Это значит, что события и задачи этих домов "
            f"тоже могут включать темы {HOUSE_SYMBOL[dh]} и становиться дополнительным способом проявления твоего Солнца."
        )
