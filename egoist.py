...
def _planet_aspect_sentence(c, planet, domain="общую тему"):
    ...
    for row in rows:
        ...
        first = f"{aspect.capitalize()} {planet_phrase_with(other)}."

        if tone == "supportive":
            meaning = rules.get("supportive", "Эта связь даёт дополнительный ресурс и помогает легче проявлять качества обеих планет.")
            if meaning.startswith(("усиливает ", "помогает ")):
                second = meaning[0].upper() + meaning[1:] + "."
            else:
                second = f"Он даёт {meaning}" if meaning else ""
        elif tone == "tense":
            meaning = rules.get("tense", "Эта связь может давать напряжение и сложности в проявлении обеих планет.")
            second = f"Он может проявляться непросто: {meaning[0].lower() + meaning[1:]}" if meaning else ""
        else:
            pos = rules.get("supportive", "усиливает качества этой планеты")
            neg = rules.get("tense", "может делать её проявление более интенсивным")
            second = (
                f"Соединение усиливает влияние {planet_instr(other)}: {pos}. "
                f"При перегрузе {neg}."
            )

        sentences.append(first + (" " + second if second else ""))

    intro = f" На {domain} особенно влияет " if len(sentences) == 1 else f" На {domain} особенно влияют несколько связей. "
    if len(sentences) == 1:
        return intro + sentences[0]
    return intro + " ".join(sentences)


SUN_HOUSE_HUMAN = {1: 'Солнце в 1 доме усиливает потребность быть заметной, самой задавать направление и влиять на происходящее. Такому человеку важно ощущать себя автором собственной жизни, а не жить только по чужим правилам.', 2: 'Солнце во 2 доме связывает самооценку с личными ресурсами, качеством жизни и способностью создавать материальную опору. Человеку важно видеть реальный результат своих усилий и чувствовать, что он умеет самостоятельно распоряжаться тем, что имеет.', 3: 'Солнце в 3 доме делает речь, обучение, обмен информацией и контакты важной частью самореализации. Такому человеку важно говорить, объяснять, учиться, передавать знания и чувствовать, что его мысли услышаны.', 4: 'Солнце в 4 доме даёт сильную потребность быть хозяйкой своего пространства и выстраивать жизнь вокруг дома, семьи и чувства надёжного тыла. Такие люди часто очень привязаны к своему гнезду, родным и личной территории, любят сами задавать правила дома и чувствовать там себя главной. Они могут быть более закрытыми или интровертными, потому что много энергии вкладывают в семью, личное пространство, отношения с близкими и создание собственной базы.', 5: 'Солнце в 5 доме усиливает творческость, желание сиять, быть замеченной и создавать что-то своё. Человеку важно получать радость от самовыражения, любви, сцены, проектов, детей или хобби, где можно проявить индивидуальность.', 6: 'Солнце в 6 доме раскрывается через мастерство, полезность и качество ежедневной работы. Такому человеку важно быть компетентным, улучшать процессы и чувствовать, что его труд действительно приносит результат.', 7: 'Солнце в 7 доме делает отношения и значимые союзы важной частью самореализации. Человек многое понимает о себе через партнёров, клиентов и взаимодействие один на один.', 8: 'Солнце в 8 доме тянет к глубоким процессам, психологии, кризисам, тайнам и темам сильной глубоких перемен. Такому человеку важно не скользить по поверхности, а понимать скрытые причины и уметь влиять на сложные ситуации.', 9: 'Солнце в 9 доме раскрывается через знания, путешествия, иностранную среду, преподавание и расширение мировоззрения. Человеку важно видеть большую картину и постоянно расти интеллектуально или духовно.', 10: 'Солнце в 10 доме усиливает амбиции, стремление к статусу и желание оставить заметный профессиональный след. Такому человеку важно видеть рост, признание и результат, который заметен не только ему самому.', 11: 'Солнце в 11 доме раскрывается через друзей, сообщества, аудиторию, технологии и идеи будущего. Человеку важно чувствовать, что он участвует в чём-то большем, чем только личная история.', 12: 'Солнце в 12 доме делает мир переживаний очень значимым и часто усиливает потребность в уединении, творчестве и работе за кадром. Такому человеку важно иметь пространство, где можно восстановиться, услышать себя и не быть постоянно на виду.'}
